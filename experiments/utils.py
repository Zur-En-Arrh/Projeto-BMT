...
def groupMessagesByConversations(connection, df, conversationSize, predatoryLabel = 'predatory_conversation'):
  ...
  dfPredatoryConversations = df[df[predatoryLabel] == True]
...
def groupMessagesByAuthor(connection, df, conversationSize):
  ...
  dfPredators = df[df['predatory_conversation'] == True]
  idPredators = list(set(dfPredators['predatory_conversation'].values))
